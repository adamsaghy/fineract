...
public final class LoanTransactionValidator {
    ...
    public void validateMerchantIssuedRefund(String json) {
        validatePaymentTransaction(json);
    }

    public void validateMerchantIssuedRefund(final Loan loan, final LocalDate transactionDate, ScheduleGeneratorDTO scheduleGeneratorDTO) {
        checkClientOrGroupActive(loan);
        validateLoanStatusIsActiveOrFullyPaidOrOverpaid(loan);
        validateActivityNotBeforeClientOrGroupTransferDate(loan, transactionDate);
        validateRepaymentTypeTransactionNotBeforeAChargeRefund(loan, LoanTransactionType.MERCHANT_ISSUED_REFUND, transactionDate);
        validateMerchantIssuedRefundNotBeforeLastTransactionDate(loan, transactionDate);
    ...
    private void validateActivityNotBeforeClientOrGroupTransferDate(final Loan loan, final LocalDate transactionDate) {
        if (loan.getClient() != null && loan.getClient().getOfficeJoiningDate() != null) {
            final LocalDate clientOfficeJoiningDate = loan.getClient().getOfficeJoiningDate();
            if (DateUtils.isBefore(transactionDate, clientOfficeJoiningDate)) {
                String errorMessage = "The date on which the merchant issued refund is made cannot be earlier than client's transfer date to this office";
                String action = "repayment.or.waiver";
                String postfix = "cannot.be.made.before.client.transfer.date";
                throw new InvalidLoanStateTransitionException(action, postfix, errorMessage, clientOfficeJoiningDate);
            }
    ...
    private void validateMerchantIssuedRefundNotBeforeLastTransactionDate(final Loan loan, final LocalDate transactionDate) {
        ...
        if (DateUtils.isAfter(lastTransactionDate, transactionDate)) {
            String errorMessage = "The date on which a merchant issued refund is made cannot be earlier than last transaction date";
            String action = "merchant.issued.refund";
